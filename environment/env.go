...
import (
	...
	"github.com/jeltevanbommel/scion-windows/pkg/fileops"
)
...
func (endhostEnv *EndhostEnvironment) Install() {
	...
	err = fileops.ReplaceStringInFile(filepath.Join(endhostEnv.ConfigPath, "bootstrapper.toml"), "{bootstrapperUrl}", endhostEnv.BootstrappingUrl)
	if err != nil {
		log.Fatal("Failed to configure url in bootstrapper.toml: ", err)
	}
