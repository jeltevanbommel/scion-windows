...
import (
	...
	"github.com/jeltevanbommel/scion-windows/external/bootstrapper/config"
	"github.com/jeltevanbommel/scion-windows/external/bootstrapper/fetcher"
	"github.com/jeltevanbommel/scion-windows/external/bootstrapper/hinting"
)
...
OuterLoop:
	for {
		...
		case ipAddr := <-b.ipHintsChan:
			...
			if err != nil {
				log.Debug("Ignore hint, not resolved", "hint", ipAddr, "err", err)
				continue
			}
			break OuterLoop
		case <-hintsTimeout:
