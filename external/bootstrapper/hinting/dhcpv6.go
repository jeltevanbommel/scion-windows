...
func getDuid(g *DHCPv6HintGenerator) (duid dhcpv6.DUID, err error) {
	if g.cfg.ClientID != "" {
		...
		return dhcpv6.DUIDFromBytes(clientID)
	}
	switch g.cfg.Duid {
	case duidLL:
		duid = &dhcpv6.DUIDLL{
...
func (op *optDNS) Code() dhcpv6.OptionCode {
	return dhcpv6.OptionDNSRecursiveNameServer
}
...
func (op *optDomainSearchList) Code() dhcpv6.OptionCode {
	return dhcpv6.OptionDomainSearchList
}
...
func (g *DHCPv6HintGenerator) dispatchDNSInfo(conv []dhcpv6.DHCPv6, dnsChan chan<- DNSInfo) {
	...
	for _, p := range conv {
		...
		for _, option := range options {
			if dns, ok := option.(*optDNS); ok {
				for _, resolver := range dns.NameServers {
		...
		for _, option := range options {
			if oDNSSL, ok := option.(*optDomainSearchList); ok {
